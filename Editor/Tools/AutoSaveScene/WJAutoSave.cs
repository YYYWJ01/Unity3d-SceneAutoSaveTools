...
namespace WJU3D {
    ...
    public class WJAutoSave : Editor {
        ...
        private static async Task SaveInterval(CancellationToken token) {
            while (!token.IsCancellationRequested) {
                ...
                if (!_config.Enabled || Application.isPlaying || UnityEditor.BuildPipeline.isBuildingPlayer || EditorApplication.isCompiling) continue;
